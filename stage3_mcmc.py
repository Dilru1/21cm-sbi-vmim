...
def load_model_new(nle_root, nc, device):
    ...
    import json
    from sbi.nle_old import load_nle
    ...
    # The returned tag becomes the chains directory: {arm}/chains/<tag>/.
    # It MUST carry the NLE seed when seed_subdir is on, or every seed_<N>
    # model writes the SAME filenames (SBC_CHAINS_..._sim{sim}_row{row}_...)
    # into chains/<scope>_<kind>/ and each run silently overwrites the last --
    # an NLE-seed ensemble would collapse to whichever member finished last,
    # with no error anywhere.
    tag = f"{scope}_{kind}"
    if subdir.name.startswith("seed_"):
        tag = f"{tag}_{subdir.name}"          # -> standard_t_nsf_seed_42
    print(f"[NLE] chains tag = {tag}", flush=True)

    # t_mean/t_std are identity for raw_t, real for standard_t -> the same
    # (t_obs - mean)/std code path is correct for both.
    return model, n_params, t_mean.astype(np.float32), t_std.astype(np.float32), tag


def load_model_legacy(nle_root, device):
    """Old single-file layout: {arm}/nle/best_mdn_nle.pt (GMM only)."""
    from sbi_project_old.sbi.nle_old import MDNLikelihood  # must still exist in sbi/nle.py
