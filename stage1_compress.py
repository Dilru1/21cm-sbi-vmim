...
class NoiseResampledDataset(torch.utils.data.Dataset):
    ...
    def __getitem__(self, i):
        ...
        return torch.from_numpy(block[j].astype(np.float32)), torch.from_numpy(
            th[:4].astype(np.float32)
        )
...
def export_memmap(compressor, get_batch_iter, n_rows, device, out_dir, t_dim, noise_scale=None):
    ...
    theta_mm = np.lib.format.open_memmap(
        os.path.join(out_dir, "theta.npy"), mode="w+", dtype=np.float32, shape=(n_rows, 4)
    )
    t_mm = np.lib.format.open_memmap(
        os.path.join(out_dir, "t.npy"), mode="w+", dtype=np.float32, shape=(n_rows, t_dim)
    )
    sim_mm = np.lib.format.open_memmap(
        os.path.join(out_dir, "original_sim_ids.npy"), mode="w+", dtype=np.int64, shape=(n_rows,)
    )
    noise_mm = np.lib.format.open_memmap(
        os.path.join(out_dir, "noise_ids.npy"), mode="w+", dtype=np.int64, shape=(n_rows, 3)
    )
    if noise_scale is not None:
        np.save(
            os.path.join(out_dir, "export_noise_scale.npy"), np.array([noise_scale], np.float32)
        )
    off = 0
    with torch.no_grad():
        for bi, (x, theta, sim, nids) in enumerate(get_batch_iter()):
            ...
            noise_mm[off : off + k] = (
                nn_.astype(np.int64)
                if nn_.ndim == 2
                else np.repeat(nn_[:, None], 3, 1).astype(np.int64)
            )
            off += k
            if bi % 100 == 0:
...
def main():
    ...
    if cfg["arm_type"] == "mlp":
        ...
        from sbi.cubes import DeterministicNoisyExport, prepare_cube_loaders
        ...
        if model_choice == "seblock":
            ...
            from sbi.cubes_grn4d import DeterministicNoisyExport, prepare_cube_loaders
    ...
    if args.no_export:
        print(
            "[NO-EXPORT] training + probes done; skipping memmap export. "
            f"RF-R2 history and loss curves saved in {dirs['nle']}",
            flush=True,
        )
        return
    ...
    raw = compressor.module if isinstance(compressor, nn.DataParallel) else compressor
    ckpt_name = (
        "learned_compressor_bestprobe.pt" if args.ckpt == "bestprobe" else "learned_compressor.pt"
    )
    ...
    if cfg["arm_type"] == "mlp":
        ...
        all_theta = export_src.theta
        all_sim = export_src.sim_ids
        all_noise = export_src.noise_ids
        loader = DataLoader(
            TensorDataset(all_t, torch.arange(len(all_t))),
            batch_size=c["save_batch_size"],
            shuffle=False,
        )
        ...
        def it():
            for xb, idx in loader:
                ...
                yield (
                    ...
                    all_theta[idx],
                    all_sim[idx],
                    all_noise[idx] if all_noise.ndim == 2 else all_noise[idx],
                )
