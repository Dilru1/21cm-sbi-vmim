...
def main():
    ...
    # --- sharded export (CNN only) ---------------------------------------
    # DeterministicNoisyExport derives its noise index from the GLOBAL
    #     (original_sim_id * total_nnoise + noise_rep + 10007*channel) % n_noise
    # so a shard produces byte-identical rows to the matching slice of an
    # unsharded export. Shards are disjoint and their union is the full table.
    ap.add_argument("--noise-start", type=int, default=0,
                    help="first noise realisation index for this shard")
    ap.add_argument("--noise-end", type=int, default=None,
                    help="one past the last noise index; None = all. When set, "
                         "the shard writes to summaries/part_<start>_<end>/ and "
                         "merge_export_shards.py concatenates them afterwards.")
    args = ap.parse_args()

    cfg = load_config(args.config, args.override)
    if args.noise_end is not None and cfg.get("arm_type", "cnn") != "cnn":
        sys.exit("--noise-start/--noise-end are CNN-only: the mlp and raw export "
                 "branches ignore them and write straight to summaries/, so N "
                 "array tasks would write the same four memmaps at once.")
    c = cfg["compressor"]
    ...
    set_all_seeds(init_seed, deterministic=False)   
    # on 04/09/2025 changed to True from False
    #settings this false make training fast?
    # The only tradeoff: re-running the same seed won't give bit-identical weights anymore (
    # floating-point reduction order varies run to run). For a seed-averaging experiment 
    # that variability is well below what you're already averaging over, so it doesn't affect the science.
    ...
    raw = compressor.module if isinstance(compressor, nn.DataParallel) else compressor
    # on 04/09/2025 changed learned_compressor_bestprobe --> learned_compressor.pt so the exportes is
    # final learned compressor not the best_point probe
    ckpt_name = "learned_compressor.pt" if args.ckpt == "bestprobe" else "learned_compressor.pt"
    ...
    if cfg["arm_type"] == "mlp":
        ...
        n_start = args.noise_start
        n_end   = args.noise_end if args.noise_end is not None else total
        ds = DeterministicNoisyExport(full_va, total, c["noise_scale"],
                                      noise_start=n_start, noise_end=n_end)

        # Sharding is triggered by --noise-end being PRESENT, not by the range
        # being partial: --noise-end 1000 still writes a part_ dir and still
        # needs the merge. That keeps the array and the merge consistent.
        out_dir = dirs["summaries"]
        if args.noise_end is not None:
            out_dir = out_dir / f"part_{n_start:05d}_{n_end:05d}"
            out_dir.mkdir(parents=True, exist_ok=True)
        print(f"[export] shard noise[{n_start},{n_end}) -> {len(ds):,} rows -> {out_dir}",
              flush=True)
        ...
        export_memmap(compressor, it, len(ds), device, str(out_dir), cfg["t_dim"],
