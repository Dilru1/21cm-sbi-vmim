...
def load_arm(spec):
    ...
    print(f"[{label}] rows={len(t):,} t_dim={t.shape[1]} "
          f"n_sims={len(np.unique(np.asarray(sims))):,} noise_scale={ns:g}", flush=True)
    return dict(label=label, t=np.asarray(t, np.float64), theta=np.asarray(theta, np.float64),
                sims=np.asarray(sims), noise_scale=ns)
...
def decompose(arm, max_sims=1500, rng=None):
    ...
    for s in uniq:
        ...
        th.append(theta[m][0])
    means = np.asarray(means); wvars = np.asarray(wvars); th = np.asarray(th)

    within = np.sqrt(wvars.mean(0))
    between = means.std(0)
    snr = between / np.maximum(within, 1e-12)
    arm.update(sim_means=means, sim_theta=th, within=within, between=between, snr=snr,
               sim_ids_used=uniq[:len(means)])
    return arm


def param_snr(arm, n_params):
    T = arm["sim_means"]; TH = arm["sim_theta"][:, :n_params]
...
def main():
    ...
    for ax, ttl, yl in zip(axes,
        ...
        ax.set_title(ttl); ax.set_xlabel("latent dim"); ax.set_ylabel(yl)
        ax.set_xticks(xs); ax.grid(alpha=0.25, axis="y"); ax.legend(fontsize=8)
    axes[2].axhline(1.0, color="k", ls="--", lw=1, alpha=0.6)
    fig.tight_layout(); fig.savefig(f"{args.out}_snr.png", dpi=130, bbox_inches="tight")
    plt.close(fig)
    ...
    fig, axes = plt.subplots(len(arms), n_params, figsize=(4.2 * n_params, 3.8 * len(arms)),
                             squeeze=False)
    for i, a in enumerate(arms):
        Z = ((a["sim_means"] - mu) / sd) @ P
        for j in range(n_params):
            ax = axes[i][j]
            sc = ax.scatter(Z[:, 0], Z[:, 1], c=a["sim_theta"][:, j], s=6, alpha=0.6,
                            cmap="viridis")
    ...
    fig.tight_layout(); fig.savefig(f"{args.out}_pca.png", dpi=130, bbox_inches="tight")
    plt.close(fig)
    ...
    if args.smear_overlay:
        ...
        for idx, s in enumerate(pick):
            ...
            for i, a in enumerate(arms):
                ...
                if args.center_islands:
                    Z = Z - Z.mean(0)
                ax.scatter(Z[:, 0], Z[:, 1], s=5, alpha=args.smear_alpha,
                            color=arm_colors[i], edgecolors="none", zorder=2 + i)
                if not args.center_islands:
                    ax.scatter(Z[:, 0].mean(), Z[:, 1].mean(), s=110, marker="*",
                               edgecolor="k", linewidth=0.6, color=arm_colors[i],
                               zorder=10 + i)
            if args.center_islands:
                ax.axhline(0, color="0.8", lw=0.5, zorder=0)
                ax.axvline(0, color="0.8", lw=0.5, zorder=0)
            th = arms[0]["sim_theta"][list(arms[0]["sim_ids_used"]).index(s)] \
                 if s in arms[0]["sim_ids_used"] else None
            ttl = f"sim {s}"
            if th is not None:
        ...
        handles = [Line2D([], [], marker="o", ls="", color=arm_colors[i],
        ...
        fig.suptitle("Noise smear at fixed theta (stars = per-sim means)", y=1.05 if nrow == 1 else 1.02)
    else:
        fig, axes = plt.subplots(1, len(arms), figsize=(5.6 * len(arms), 5.0),
                                 squeeze=False)
        for i, a in enumerate(arms):
            ax = axes[0][i]
            for k, s in enumerate(pick):
                ...
                ax.scatter(Z[:, 0].mean(), Z[:, 1].mean(), s=90, marker="*",
                           edgecolor="k", color=cmap(k % 10), zorder=5)
            ax.set_title(f"{a['label']} (ns={a['noise_scale']:g})\n"
                         f"{len(pick)} sims, all noise reps")
            ax.set_xlabel("PC1"); ax.set_ylabel("PC2"); ax.grid(alpha=0.25)
        fig.suptitle("Noise smear at fixed theta (stars = per-sim means)", y=1.0)

    fig.tight_layout(); fig.savefig(f"{args.out}_smear.png", dpi=130, bbox_inches="tight")
    plt.close(fig)
    ...
    if args.corner:
        ...
        ncols = t_dim - 1
        fig, axes = plt.subplots(len(arms) * (t_dim - 1), ncols,
        ...
        for ai, a in enumerate(arms):
            ...
            for (i, j) in pairs:
                ax = axes[r0 + (j - 1)][i]
                ax.axis("on")
                sc = ax.scatter(T[:, i], T[:, j], c=col, s=7, alpha=0.7,
            ...
            axes[r0][0].set_title(f"{a['label']} (ns={a['noise_scale']:g})  "
                                  f"— coloured by {names[cb_idx]}",
                                  fontsize=9, loc="left")
    ...
    if gp_name is not None and gp_name in names:
        ...
        def best_dim_for(a, j):
            T = a["sim_means"]
            c = [abs(np.corrcoef(T[:, i], a["sim_theta"][:, j])[0, 1])
        ...
        for i, a in enumerate(arms):
            ...
            lvl_col = {lv: cmap_lvl(k / max(len(levels) - 1, 1))
            ...
            for lv in levels:
                ...
                axt.hist(yl, bins=bins, histtype="stepfilled", alpha=0.45,
                         color=lvl_col[lv], edgecolor="none", zorder=3,
            ...
            axt.text(0.05, 0.92, text_label, transform=axt.transAxes, fontsize=8,
                     verticalalignment='top', bbox=dict(boxstyle='round,pad=0.5', 
                                                       facecolor=box_color, 
                                                       edgecolor=border_color, 
                                                       alpha=0.9), zorder=10)

            axt.set_title(f"{a['label']} (ns={a['noise_scale']:g})\n"
            ...
            if i == len(arms) - 1:
                axt.legend(fontsize=7, loc="upper right", frameon=True, facecolor='white', framealpha=0.8)
            ...
            jitterx = (np.random.default_rng(0).uniform(-1, 1, len(true))
                       * 0.01 * (levels.max() - levels.min() + 1e-9))
            
            axb.scatter(true + jitterx, y, s=12, alpha=0.4,
            ...
            for lv, m in zip(levels, [y[np.isclose(true, lv)].mean() for lv in levels]):
                ...
                axb.fill_between([lv - 0.04, lv + 0.04], m - noise, m + noise,
                                 color=lvl_col[lv], alpha=0.2, edgecolor=lvl_col[lv], 
        ...
        fig.suptitle(f"The Jitter Dequantization Effect on '{gp_name}' in Latent Space",
    ...
    for i, a in enumerate(arms):
        ...
        ax.set_xticks(range(n_params)); ax.set_xticklabels(names, rotation=45, ha="right")
        ax.set_yticks(range(t_dim)); ax.set_yticklabels([f"t{k}" for k in range(t_dim)])
        ax.set_title(f"{a['label']} (ns={a['noise_scale']:g})")
        plt.colorbar(im, ax=ax, fraction=0.046)
    fig.suptitle("|corr(latent dim, parameter)|", y=1.02)
    fig.tight_layout(); fig.savefig(f"{args.out}_corr.png", dpi=130, bbox_inches="tight")
    plt.close(fig)
    ...
    snrs = {}
    for a in arms:
        p = param_snr(a, n_params); snrs[a["label"]] = p
