...
from sbi.seeding import resolve_seeds, set_all_seeds, seed_worker, make_generator
...
def get_split_sims(cfg, split_seed):
    ...
    if cfg["arm_type"] == "mlp":
        ...
        out = split_by_sim(src.t, src.theta, src.sim_ids, src.noise_ids,
                           c["val_frac"], split_seed)
...
def build_models(cfg, device):
    ...
    if cfg["arm_type"] == "mlp":
        ...
        comp = SummaryCompressor(n_input=n_input, t_dim=cfg["t_dim"],
                                 dense_layers=tuple(c["dense_layers"]),
                                 dropout=c["dropout"], activation=c["activation"],
        ...
        comp = C(t_dim=cfg["t_dim"], n_params=cfg["n_params"],
                 direct=bool(c.get("direct_regression", False))).to(device)
    head = build_head(c["vmim_head"], cfg["t_dim"], cfg["n_params"],
                      c["n_mix_head"], c["hidden_head"]).to(device)
    return comp, head
...
def weight_digest(cfg, init_seed, device):
    ...
    ws = [p for p in comp.state_dict().values()] + \
         [p for p in head.state_dict().values()]
    return digest(ws)
...
def main():
    ...
    ap.add_argument("--with-batches", type=int, default=0, metavar="N",
    ...
    splits = {}
    for s in args.seeds:
        cfg_s = dict(cfg); cfg_s["compressor"] = dict(c); cfg_s["compressor"]["init_seed"] = s
        ss, _ = resolve_seeds(cfg_s["compressor"])
        tr, va = get_split_sims(cfg_s, ss)
        splits[s] = (digest([tr]), digest([va]), len(tr), len(va))
        print(f"    seed {s}: split_seed={ss} train={splits[s][2]} val={splits[s][3]} "
              f"hash={splits[s][0]}", flush=True)
    ref = splits[args.seeds[0]][:2]
    ok &= report("all init_seeds give the identical train/val split",
    ...
    ok &= report("every init_seed gives DIFFERENT weights",
    ...
    ok &= report(f"re-running init_seed={s0} reproduces the same weights",
                 again == digs[s0],
    ...
    if args.with_batches:
        ...
        ok &= report("batch/augmentation stream differs across init_seeds",
                     bd[a] != bd[b],
                     "if this FAILS, worker_init_fn/generator is not wired in")
        rep = batch_digest(cfg, base_split, a, args.with_batches)
        ok &= report(f"batch stream for init_seed={a} is reproducible",
                     rep == bd[a],
