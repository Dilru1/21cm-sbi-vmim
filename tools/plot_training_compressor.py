...
def load_arm(spec, P):
    ...
    if rf.shape[1] < P + 1:
        sys.exit(f"[FATAL] '{label}': rf_r2_history has {rf.shape[1]} cols, "
                 f"need >= {P + 1} for {P} parameters")
    sig = _load("sigma_history.npy")
    if sig is not None and sig.shape[1] < P + 1:
        print(f"[warn] '{label}': sigma_history has {sig.shape[1]} cols; ignoring",
              file=sys.stderr)
        sig = None
...
def main():
    ...
    ap.add_argument("--cmap", default="coolwarm",
                    help="diverging colormap: coolwarm, RdBu_r, bwr, PuOr")
    ap.add_argument("--colwidth", type=float, default=1.95,
    ...
    gs = GridSpec(len(rows), P, figure=fig,
                  height_ratios=[hgt[r] for r in rows], hspace=0.45, wspace=0.12)
    ...
    for p in range(P):
        ...
        for a, c in zip(arms, colors):
            ax.plot(a["ep"], a["r2"][:, p], "-o", color=c, mfc="white",
                    mew=0.9, label=a["label"], zorder=3, clip_on=False)
            ...
            ax.plot(a["ep"][b], a["r2"][b, p], "*", color=c, ms=9,
                    mec="0.15", mew=0.5, zorder=4, clip_on=False)
    ...
    if have_sigma:
        smax = max(np.nanmax(a["sigma"][:, 1:1 + P])
                   for a in arms if a["sigma"] is not None)
        ...
        for p in range(P):
            ...
            for a, c in zip(arms, colors):
                if a["sigma"] is None:
                    continue
                ax.plot(a["sigma"][:, 0], a["sigma"][:, 1 + p], "-o", color=c,
                        mfc="white", mew=0.9, label=a["label"], zorder=3, clip_on=False)
            ...
            if p in dq:
                ax.axhline(dq[p] / np.sqrt(3.0), color="#B2182B", lw=0.9,
                           ls="-.", zorder=2)
            ...
            if p == 0:
                ...
                ax.text(0.02, PRIOR_SIGMA, "prior", transform=tr, va="bottom",
                        fontsize=6.5, color="0.25")
                ax.text(0.02, args.sigma_floor, "floor", transform=tr, va="bottom",
                        fontsize=6.5, color="0.55")
            else:
                ax.tick_params(labelleft=False)
            if p in dq:
                ax.text(0.02, dq[p] / np.sqrt(3.0), "dequant",
    ...
    if have_loss:
        ...
        h_spl = [Line2D([], [], color="0.3", lw=1.4, ls="-"),
                 Line2D([], [], color="0.3", lw=1.2, ls="--")]
        l1 = ax.legend(h_arm, [a["label"] for a in arms], loc="upper right",
                       borderaxespad=0.4)
        ax.add_artist(l1)
        ax.legend(h_spl, ["train", "validation"], loc="upper right",
                  bbox_to_anchor=(1.0, 0.70), borderaxespad=0.4)
    ...
    table("RF-R2 at best-probe checkpoint (the one exported):",
          lambda a: a["r2"][int(np.argmax(a["r2"].mean(axis=1)))])
    if have_sigma:
        table(f"final sigma(theta_p|t)  [prior {PRIOR_SIGMA:.3f}]:",
    ...
    with open(tex, "w") as fh:
        ...
        for a in arms:
            ...
            fh.write(f"{a['label']} & " + " & ".join(f"{x:.3f}" for x in v)
                     + r" \\" + "\n")
        fh.write("\\bottomrule\n\\end{tabular}\n")
