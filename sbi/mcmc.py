...
def batch_loglik(model, theta, target_t, device, bs, use_prior, ti, mi):
    ...
    target_t = np.asarray(target_t, np.float32)
    ...
    if use_prior and theta.shape[1] > max(ti, mi):
        mask |= (theta[:, mi] > (1.98 - 1.88 * theta[:, ti])) | (
            theta[:, mi] < (0.90 - 1.88 * theta[:, ti])
        )
...
def run_chain(model, target_t, n_params, device, mc, seed):
    ...
    up, ti, mi, bs = (
        mc["use_original_prior"],
        mc["wedge_tau_idx"],
        mc["wedge_mmin_idx"],
        mc["loglik_batch_size"],
    )
    ...
    for step in range(steps - 1):
        if step % 50 == 0:
            ...
            print(
                f"  step {step}/{steps - 1} best {np.max(fin) if len(fin) else -np.inf:.3f}",
                flush=True,
            )
        ...
        for sub in range(2):
            ...
            proposal = chain[step, partner] + z[:, None] * (
                chain[step, lo:hi] - chain[step, partner]
            )
            pl = batch_loglik(model, proposal, target_t, device, bs, up, ti, mi)
            with np.errstate(invalid="ignore", divide="ignore", over="ignore"):
                lar = np.nan_to_num(
                    (n_params - 1) * np.log(z) + pl - logp[step, lo:hi],
                    nan=-np.inf,
                    posinf=np.inf,
                    neginf=-np.inf,
                )
