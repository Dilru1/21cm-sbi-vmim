...
def get_stats_global(signal_paths, noise_paths, sim_ids, noise_scale):
    ...
    all_noise_var = 0.0
    n_noise_files = 0
    for s_path, n_path in zip(signal_paths, noise_paths, strict=False):
    ...
    global_std = np.float32(
        np.sqrt(combined.var() + (noise_scale**2) * (all_noise_var / n_noise_files) + 1e-12)
    )
...
class CubeDataset(Dataset):
    def __init__(
        ...
        self.params_norm[:, 0] = (np.log10(self.params_norm[:, 0] / 0.00192) - (-1.0)) / (
            1.0 - (-1.0)
        )
        self.params_norm[:, 1] = (np.log10(self.params_norm[:, 1]) - np.log10(738.9)) / (
            np.log10(10504.7) - np.log10(738.9)
        )
        # column 2 (rHS) is already in [0,1]; left untouched. Dequantization /
        # margin handling happens inside the VMIM head, not here.
        self.params_norm[:, 3] = (self.params_norm[:, 3] - 8.0) / (9.6 - 8.0)
        ...
        self.signals = [
            np.fromfile(p, dtype=np.float64)
            .astype(np.float32)
            .reshape(-1, 32, 32, 32)[:9827][self.sim_ids]
            for p in s_paths
        ]
        self.noises = [
            np.fromfile(p, dtype=np.float64).astype(np.float32).reshape(-1, 32, 32, 32)
            for p in n_paths
        ]
...
def prepare_cube_loaders(cfg_data, c, seed, val_frac):
    ...
    if model_choice == "conv4d":
        means, stds = get_stats_global(
            cfg_data["s_paths"], cfg_data["n_paths"], sim_ids, c["noise_scale"]
        )
    else:
    ...
    full_train_dataset = CubeDataset(
        cfg_data["s_paths"],
        cfg_data["n_paths"],
        cfg_data["params_path"],
        cfg_data["sim_ids_path"],
        means,
        stds,
        augment=c["augment"],
...
class DeterministicNoisyExport(Dataset):
    ...
    def __getitem__(self, global_idx):
        ...
        for ch in range(3):
            ...
            noise_idx = (
                original_sim_id * self.total_n_noise_per_sim + noise_rep + 10007 * ch
            ) % n_noise_total
            noisy_cube = (
                self.base.signals[ch][sim_idx] + self.noise_scale * self.base.noises[ch][noise_idx]
            )
