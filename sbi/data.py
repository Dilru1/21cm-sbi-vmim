...
def load_source(path, mmap=True):
    ...
    if p.is_file() and p.suffix == ".npz":
        d = np.load(p)
        theta = d["theta"].astype(np.float32); t = d["t"].astype(np.float32)
...
def load_sbc_sims(path):
    ...
    return np.array(sorted({int((x.item() if hasattr(x, "item") else x)["sim"]) for x in arr}), np.int64)


def load_sbc_targets(path):
    arr = np.load(path, allow_pickle=True)
    return [{"sim": int((x.item() if hasattr(x, "item") else x)["sim"]),
             "offset": int((x.item() if hasattr(x, "item") else x)["offset"])} for x in arr]


def split_by_sim(theta, t, sim_ids, noise_ids=None, val_frac=0.1, seed=42):
    rng = np.random.default_rng(seed)
    uniq = np.unique(sim_ids); rng.shuffle(uniq)
    ...
    return (theta[tr][it], t[tr][it], sim_ids[tr][it], ni[tr][it],
            theta[va][iv], t[va][iv], sim_ids[va][iv], ni[va][iv],
            np.array(sorted(train)), np.array(sorted(val)))
